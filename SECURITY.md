...
The primary Render static site and Netlify fallback are expected to enforce equivalent baseline response protections: `nosniff`, clickjacking denial, strict referrer behavior, restricted browser permissions, COOP/CORP isolation, HSTS, and bounded cache policies. The repository keeps these rules in `render.yaml` and `netlify.toml`.

Production smoke checks always enforce deployment revision, routes, canonical metadata, crawler files, PWA assets, and project evidence. Security-header drift is reported as a warning while the existing Render static site remains independently configured. Once that service is managed by the committed Blueprint policy, set `REQUIRE_SECURITY_HEADERS=true` in the smoke workflow to make the complete header contract a hard failure.
